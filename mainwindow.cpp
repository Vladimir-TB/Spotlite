    ...
    //,_smobile(NULL)
{
    ...
    // Forceer eenmalig licht thema en SunnyEx-skin als standaard voor deze release
    if (!s->value("ui/theme.migrated2026", false).toBool()) {
    ...
    connect(ui->actionDonker_thema, SIGNAL(toggled(bool)), this, SLOT(on_actionDonker_thema_toggled(bool)));
    ...
    setSkin( s->value("skin", "SunnyEx").toString() );
...
void MainWindow::catPopupNewFilter()
{
    ...
    QString skin = sl->settings()->value("skin", "SunnyEx").toString();
    setSkin(skin);
}
...
void MainWindow::on_actionOver_SpotLite_triggered()
{
    QMessageBox::about(
        this,
        SPOTLITE_VERSION,
        tr("Door spotnet team 2026 , voor hulp en vragen zie spotnet.forum")
    );
}
...
{
    QSettings *s = sl->settings();
    QString currentskin = s->value("skin", "SunnyEx").toString();
...
void MainWindow::setFonts()
{
    ...
    if ( s->value("listfont").isValid() )
    {
        ...
        QWebEngineProfile::defaultProfile()->settings()->setFontSize(QWebEngineSettings::MinimumFontSize, sf.pointSize() );
...
void MainWindow::on_actionDonker_thema_toggled(bool checked)
{
...
QString MainWindow::darkStyleSheet() const
{
    return QString::fromLatin1(
        "* { selection-background-color: #3a7afe; selection-color: #f4f6fb; }\n"
        "QMainWindow, QWidget { background: #0f141c; color: #e6ebf1; font-size: 10pt; }\n"
        "QMenuBar, QMenu { background: #161d27; color: #e6ebf1; border: 1px solid #202a38; }\n"
        "QMenuBar::item { background: transparent; padding: 6px 10px; color: #e6ebf1; }\n"
        "QMenuBar::item:selected { background: #273449; color: #f4f6fb; }\n"
        "QMenuBar::item:pressed { background: #1e2736; }\n"
        "QMenu::item:selected { background: #273449; }\n"
        "QToolTip { background: #161d27; color: #e6ebf1; border: 1px solid #202a38; }\n"
        "QTabWidget::pane { border: 1px solid #202a38; top: -1px; }\n"
        "QTabBar::tab { background: #161d27; color: #dfe6ee; padding: 6px 12px; margin-right: 1px; }\n"
        "QTabBar::tab:selected { background: #273449; }\n"
        "QTabBar::tab:hover { background: #1e2736; }\n"
        "QTableView, QTreeWidget, QListWidget { background: #0f141c; alternate-background-color: #131a24; selection-background-color: #273449; selection-color: #f4f6fb; border: 1px solid #202a38; }\n"
        "QHeaderView::section { background: #161d27; color: #dfe6ee; border: 1px solid #202a38; padding: 6px; }\n"
        "QLineEdit, QTextEdit, QPlainTextEdit { background: #0b1017; color: #e6ebf1; border: 1px solid #253041; border-radius: 4px; padding: 6px; }\n"
        "QPushButton { background: #1b2432; color: #e6ebf1; border: 1px solid #2c394c; border-radius: 6px; padding: 6px 12px; }\n"
        "QPushButton:hover { background: #223047; }\n"
        "QPushButton:pressed { background: #1a2434; }\n"
        "QCheckBox, QRadioButton { spacing: 6px; }\n"
        "QSplitter::handle { background: #161d27; width: 6px; }\n"
        "QStatusBar { background: #0b1017; color: #e6ebf1; border-top: 1px solid #202a38; }\n"
        "QScrollBar:vertical, QScrollBar:horizontal { background: #131a24; width: 12px; height: 12px; margin: 2px; }\n"
        "QScrollBar::handle:vertical, QScrollBar::handle:horizontal { background: #273449; border-radius: 6px; min-height: 24px; min-width: 24px; }\n"
...
{
    return QString::fromLatin1(
        "* { selection-background-color: #2b7cff; selection-color: #ffffff; }\n"
        "QMainWindow, QWidget { background: #f4f6fa; color: #1c2430; font-size: 10pt; }\n"
        "QMenuBar, QMenu { background: #ffffff; color: #1c2430; border: 1px solid #d9dfe7; }\n"
        "QMenu::item:selected { background: #e7edf7; }\n"
        "QToolTip { background: #ffffff; color: #1c2430; border: 1px solid #d9dfe7; }\n"
        "QTabWidget::pane { border: 1px solid #d9dfe7; top: -1px; }\n"
        "QTabBar::tab { background: #edf1f7; color: #1c2430; padding: 6px 12px; margin-right: 1px; }\n"
        "QTabBar::tab:selected { background: #ffffff; border-bottom: 2px solid #2b7cff; }\n"
        "QTableView, QTreeWidget, QListWidget { background: #ffffff; alternate-background-color: #f4f6fa; selection-background-color: #e7edf7; selection-color: #0f1824; border: 1px solid #d9dfe7; }\n"
        "QHeaderView::section { background: #f0f3f8; color: #1c2430; border: 1px solid #d9dfe7; padding: 6px; }\n"
        "QLineEdit, QTextEdit, QPlainTextEdit { background: #ffffff; color: #1c2430; border: 1px solid #c9d1dc; border-radius: 4px; padding: 6px; }\n"
        "QPushButton { background: #e7ecf4; color: #1c2430; border: 1px solid #c9d1dc; border-radius: 6px; padding: 6px 12px; }\n"
        "QPushButton:hover { background: #dfe6f1; }\n"
        "QPushButton:pressed { background: #d2dae8; }\n"
        "QCheckBox, QRadioButton { spacing: 6px; }\n"
        "QSplitter::handle { background: #e7ecf4; width: 6px; }\n"
        "QStatusBar { background: #f0f3f8; color: #1c2430; border-top: 1px solid #d9dfe7; }\n"
        "QScrollBar:vertical, QScrollBar:horizontal { background: #f0f3f8; width: 12px; height: 12px; margin: 2px; }\n"
        "QScrollBar::handle:vertical, QScrollBar::handle:horizontal { background: #c9d1dc; border-radius: 6px; min-height: 24px; min-width: 24px; }\n"
        "QScrollBar::add-line, QScrollBar::sub-line { background: transparent; }\n"
    );
}
...
void MainWindow::applyTheme(const QString &mode)
{
    ...
    if (normalized == "dark")
    {
        ...
        if (ui->actionDonker_thema)
            ui->actionDonker_thema->setChecked(true);
    }
    else
    {
        qApp->setStyleSheet(lightStyleSheet());
        if (ui->actionDonker_thema)
            ui->actionDonker_thema->setChecked(false);
    }
