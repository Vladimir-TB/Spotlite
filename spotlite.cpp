...
//            QApplication::processEvents();
        ...
        auto decodeSubject = [&](const QByteArray &raw) -> QString {
            QByteArray cleaned = raw.trimmed().replace("?= =?", "?==?");
            if (cleaned.contains("=?"))
                return _decodeUtf8(cleaned);
            // Als geen encoding-header: eerst proberen UTF-8, als dat invalid is, cp1252/Latin1
            QString utf = QString::fromUtf8(cleaned);
            if (utf.count(QChar::ReplacementCharacter) > 0)
                return QString::fromLatin1(cleaned);
            return utf;
        };

        QString subjDecoded = decodeSubject(l[1]);
        l[1] = subjDecoded.toUtf8();
...
//            _checkDuplicatesQuery.bindValue(1, msgid);
        ...
        if (ero)
        {
            ...
            /*if (subj_tag[0].contains("?=") && subj_tag[0].contains("=?"))
                _einsertQuery.bindValue(3, _decodeUtf8(subj_tag[0].trimmed().replace("?= =?", "?==?")) );
            else*/
                _einsertQuery.bindValue(3, QString(subjDecoded));
        ...
        else
        {
            ...
            /*if (subj_tag[0].contains("?=") && subj_tag[0].contains("=?"))
                _insertQuery.bindValue(3, _decodeUtf8(subj_tag[0].trimmed().replace("?= =?", "?==?")) );
            else*/
                _insertQuery.bindValue(3, QString(subjDecoded) );
...
{
    QString r;
    if (s.contains("=?UTF-8?B?"))
    {
        r = QString::fromLatin1(s);
        int startpos;
    ...
    // Als UTF-8 fallback mislukt of er zijn veel �, neem cp1252/Latin1
    if (r.count(QChar::ReplacementCharacter) > 1)
        r = QString::fromLatin1(s);
