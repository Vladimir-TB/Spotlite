...
class ImageListPage : public QWebEnginePage
{
    Q_OBJECT
public:
    ...
    QWidget(parent), _index(0), _total(0), _pagesize(25), _sl(NULL), _model(NULL)
{
    ...
    /* Webbrowser spul */
    _web = new QWebEngineView(this);
    auto *page = new ImageListPage(_web);
    _web->setPage(page);
...
void SpotImageList::updateHTML()
{
    ...
    _web->setHtml(QString::fromUtf8(html));
}

void SpotImageList::_downloadRequested(QWebEngineDownloadItem *download)
{
    if (!download)
        return;
